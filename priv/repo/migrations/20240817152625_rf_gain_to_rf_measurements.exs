defmodule RFDataViewer.Repo.Migrations.RfGainToRfMeasurements do
  use Ecto.Migration

  def change do
    rename_fkey_table(from: :rf_gain, to: :rf_measurements, field: :rf_data_set_id)
    rename_pkey_table(from: :rf_gain, to: :rf_measurements)
    rename_seq_table(from: :rf_gain, to: :rf_measurements, field: :id)
    rename_index_table(from: :rf_gain, to: :rf_measurements, fields: :rf_data_set_id)
    rename table(:rf_gain), to: table(:rf_measurements)
    rename table(:rf_measurements), :gain, to: :value
    alter table(:rf_measurements), do: add(:type, :string)
    create index(:rf_measurements, [:type])

    flush()

    RFDataViewer.Repo.update_all("rf_measurements", set: [type: "gain"])
